...
{
    public class GetMessageBySenderIdAndReciverIdQuery
    {
        ...
        public int newReciverId { get; set; }

        public GetMessageBySenderIdAndReciverIdQuery(ApplicationDbContext dataBase, IMapper mapper)
        {
            _dataBase = dataBase;
            _mapper = mapper;
        }
        ...
        public List<GetMessageModel> Handle()
        {
            var message = _dataBase.Messages
                .Where(a => a.Sender_Id == newSenderId && a.Reciver_Id == newReciverId).OrderBy(c => c.MessageId);
